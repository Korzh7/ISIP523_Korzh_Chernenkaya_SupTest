...
## 10. Повторное тестирование

### 10.1 Результаты повторного тестирования

После исправления всех багов проведено повторное тестирование:

| Тип тестирования | Результат |
|------------------|-----------|
| Автоматизированное | 7/7 ✅ |
| Ручное функциональное | 11/11 ✅ |
| Ручное нефункциональное | 4/4 ✅ |

### 10.2 Вывод

Все баги успешно исправлены. Приложение работает корректно во всех тестовых сценариях.

## 11. Заполнение тестовых сценариев

### 11.1 Позитивные тесты

| № | Сценарий | Входные данные | Ожидаемый результат | Фактический результат | Статус |
|---|----------|----------------|---------------------|----------------------|--------|
| 1 | Шифрование английского текста | text="HELLO", key="KEY" | "RIJVS" | "RIJVS" | ✅ PASS |
| 2 | Шифрование русского текста | text="ПРИВЕТ", key="КЛЮЧ" | Шифротекст отличен от оригинала | "ЪЬЕЪНК" | ✅ PASS |
| 3 | Дешифрование английского текста | cipher="RIJVS", key="KEY" | "HELLO" | "HELLO" | ✅ PASS |
| 4 | Дешифрование русского текста | cipher="ЪЬЕЪНК", key="КЛЮЧ" | "ПРИВЕТ" | "ПРИВЕТ" | ✅ PASS |
| 5 | Обратимость (encrypt→decrypt) | text="Test123!", key="secret" | Исходный текст | "Test123!" | ✅ PASS |
| 6 | Ключ короче текста | text="LONGTEXT", key="KEY" | Циклическое использование | Циклическое | ✅ PASS |
| 7 | Ключ длиннее текста | text="SHORT", key="VERYLONG" | Используется префикс ключа | Используется | ✅ PASS |
| 8 | Сохранение регистра | text="HeLlO", key="Key" | Регистр сохранён | "RiOwS" (регистр сохранён) | ✅ PASS |
| 9 | Сохранение пробелов | text="HELLO WORLD", key="KEY" | Пробелы сохранены | "RIJVS DQTMZ" | ✅ PASS |
| 10 | Сохранение знаков препинания | text="Hello, World!", key="key" | Знаки сохранены | "Rijvs, Dqvmg!" | ✅ PASS |
| 11 | Пустой текст | text="", key="key" | "" | "" | ✅ PASS |
| 12 | Сдвиг = 0 | text="TEST", key="AAAA" | "TEST" | "TEST" | ✅ PASS |

### 11.2 Негативные тесты

| № | Сценарий | Входные данные | Ожидаемый результат | Фактический результат | Статус |
|---|----------|----------------|---------------------|----------------------|--------|
| 13 | Null текст | text=null, key="key" | ArgumentNullException | ArgumentNullException | ✅ PASS |
| 14 | Null ключ | text="test", key=null | ArgumentNullException | ArgumentNullException | ✅ PASS |
| 15 | Пустой ключ | text="test", key="" | ArgumentException | ArgumentException | ✅ PASS |
| 16 | Ключ с цифрами | text="test", key="k1y" | ArgumentException | ArgumentException | ✅ PASS |
| 17 | Ключ с пробелами | text="test", key="my key" | ArgumentException | ArgumentException | ✅ PASS |

### 11.3 Тесты нефункциональных требований

| № | Сценарий | Описание | Ожидаемый результат | Фактический результат | Статус |
|---|----------|----------|---------------------|----------------------|--------|
| 18 | Скорость шифрования | Шифрование 10000 символов | < 1 секунды | ~0.3 сек | ✅ PASS |
| 19 | Всплывающие подсказки | Наведение на поле ключа | Отображение подсказки | Отображается | ✅ PASS |
| 20 | Обработка исключений | Ввод некорректного ключа | Информативное сообщение | MessageBox с ошибкой | ✅ PASS |

### 11.4 Итог тестирования

| Категория | Всего | PASS | FAIL |
|-----------|-------|------|------|
| Позитивные | 12 | 12 | 0 |
| Негативные | 5 | 5 | 0 |
| Нефункциональные | 3 | 3 | 0 |
| **Итого** | **20** | **20** | **0** |


## 12. Заключительная информация

### 12.1 Информация о проекте

| Поле | Значение |
|------|----------|
| **Название проекта** | VigenereCipherApp |
| **Вариант** | №2 - Шифр Виженера |
| **Разработчик** | [Корж Черненькая] |
| **Группа** | 523 |
| **Дата сдачи** | 28.04.2026 |
| **Репозиторий** | [https://github.com/Korzh7/ISIP523_Korzh_Chernenkaya_SupTest](https://github.com/Korzh7/ISIP523_Korzh_Chernenkaya_SupTest) |

### 12.2 Описание предметной области

**Шифр Виженера** — полиалфавитный шифр подстановки, использующий ключевое слово для определения сдвига каждой буквы. Относится к группе шифров, где одна и та же буква открытого текста может отображаться в разные буквы шифротекста в зависимости от позиции.

**Математическое описание:**
- Шифрование: Cᵢ = (Pᵢ + Kᵢ) mod N
- Дешифрование: Pᵢ = (Cᵢ - Kᵢ + N) mod N

### 12.3 Итоги комплексного тестирования

#### Применяемые виды тестирования

| Вид тестирования | Описание | Результат |
|------------------|----------|-----------|
| **Модульное** | Тестирование отдельных методов Encrypt/Decrypt | ✅ 7/7 |
| **Интеграционное** | Взаимодействие UI с бизнес-логикой | ✅ Успешно |
| **Функциональное** | Проверка требований к шифру | ✅ 20/20 |
| **Нефункциональное** | Производительность, UI, безопасность | ✅ Успешно |
| **Регрессионное** | После исправления багов | ✅ Успешно |

#### Применяемые техники тестирования

| Техника | Применение |
|---------|------------|
| **TDD (Test-Driven Development)** | Тесты написаны до реализации |
| **White-box тестирование** | Анализ покрытия ветвей алгоритма |
| **Black-box тестирование** | Тестирование через UI |
| **Эквивалентное разбиение** | Проверка граничных значений |
| **Анализ граничных значений** | Сдвиг = 0, пустой ключ, null |
| **Позитивное/негативное тестирование** | Корректные и некорректные входные данные |

### 12.4 История коммитов

| № | Описание |
|---|----------|
| 1 |  Initial commit: базовый проект |
| 2 |  Пункт 1-2: Анализ и диаграмма |
| 3 |  Пункт 3: Тестовые сценарии |
| 4 |  Пункт 4: Реализация шифра и тесты |
| 5 |  Пункт 4: Описание в README |
| 6 |  Пункт 5: Графическое приложение |
| 7 |  Пункт 6: Отладка |
| 8 |  Пункт 7-11: Тестирование |
| 9 |  Пункт 12: Финальная документация |

### 12.5 Выводы по итогам работы

В ходе выполнения практической работы были решены следующие задачи:

1. **Реализован шифр Виженера** с поддержкой русского и английского алфавитов
2. **Разработано графическое приложение** на WPF с удобным интерфейсом
3. **Написаны автоматизированные тесты** (MSTest, 7 тестов, все пройдены)
4. **Проведена отладка** с использованием инструментов Visual Studio
5. **Составлены баг-репорты** и исправлены обнаруженные дефекты
6. **Проведено ручное тестирование** (20 тестов, все пройдены)

**Итоговая оценка качества:** Приложение работает стабильно, все требования выполнены, баги отсутствуют.

---

**📌 Ссылка на репозиторий:** [https://github.com/Korzh7/ISIP523_Korzh_Chernenkaya_SupTest/tree/Pr7.2](https://github.com/Korzh7/ISIP523_Korzh_Chernenkaya_SupTest/tree/Pr7.2)
