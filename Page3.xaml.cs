    ...
    public partial class Page3 : Page
    {
        ...
        private void btnCalculate3_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    txtError3.Text = "❌ Ошибка: x0 не может быть равен 0";
                    return;
                }
                ...
                for (double x = x0; x <= xk + 1e-10; x += dx)
                {
                    ...
                    if (underRoot < 0)
                    {
                        txtError3.Text = $"❌ Ошибка: Под корнем отрицательное число при x={x:F3}";
                        return;
                    }

                    double y = (0.01 * b * c) / x + Math.Cos(Math.Sqrt(underRoot));
